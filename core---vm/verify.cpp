...
#define SET_ERROR(code, pos) return setVMImgError(img, code, pos)
...
#define IS_ALIGNED(sz) (((sz)&7) == 0)

#define FOR_SECTIONS()                                                                             \
    VMImageSectionAndData *sect, *next;                                                                   \
    for (sect = (VMImageSectionAndData *)img->dataStart;                                                  \
...
static VMImage *countSections(VMImage *img) {
    ...
    while (p < img->dataEnd) {
        ...
        CHECK(IS_ALIGNED(sect->size), 1002);
    ...
    img->sections = ALLOC_ARRAY(VMImageSectionAndData *, img->numSections);
...
VMImage *loadVMImage(void *data, unsigned length) {
    ...
    CHECK_AT(IS_ALIGNED((uintptr_t)data), 1000, 0);
    CHECK_AT(IS_ALIGNED(length), 1001, 0);
